...
class API {
    ...
    // Define APIError enum with new cases
    enum APIError: Error {
        case invalidURL
    ...
    static func setUserTelescopes(telescope: TelescopeRequest, bearerToken: String, completion: @escaping (Result<[TelescopeProfile], Error>) -> Void) {
        // Create URL
        guard let url = URL(string: "http://192.168.0.111:8080/telescopes/save-telescope") else {
            completion(.failure(APIError.invalidURL))
            return
        }
        ...
        // Encode TelescopeProfile to JSON
        do {
            let jsonData = try JSONEncoder().encode(telescope)
